...
主题资源只来自用户提供的本地文件或 Codex 生成结果。项目不会修改 Codex 应用文件，也不会从远程地址加载主题资源。
...
Theme resources come only from local files supplied by the user or generated by Codex. The project does not patch Codex application files or load theme resources from remote URLs.
